...
p_load(stringr, ggplot2, pbapply, dplyr) # load or install+load required packages.
...
polyGen<-function(locusTable,readCounts,p_thresh=0.001,epsilon=0.01){
...
genoSetup<-function(genoData,epsilon=0.01,p_thresh=0.001){
...
exportGenepop<-function(polygenResults,locusTable,exportParalogs=FALSE,sampleWhitelist=NULL,locusWhitelist=NULL,sampleBlacklist=NULL,locusBlacklist=NULL,filename="polygenResults.genepop"){
  ...
  #add "Pop" before the start of each SILLY. Note this assumes a SILLY represents a pop.
  #I'd like to use group_walk or similar, but those are experimental, so this seems more stable, albeit clunky.
  genepopBases<- data.frame(genepopBases,check.names = FALSE) %>% #next line only works on df,tibble, etc.; do not convert column names
    tibble::rownames_to_column(var = "samples") %>% # getting sample info
    tidyr::separate(samples, into = c("silly", "fish"), sep = "_", remove = FALSE) %>% # split out silly to be used as unique identifier
    dplyr::group_by(silly) %>% 
    tidyr::uncount((row_number() == 1) + 1) %>% # unweight dataframe, in this case acts on grouped variable (i.e., silly) to find and duplicate the row containing the first unique silly
    dplyr::ungroup() %>%
    dplyr::mutate(samples = replace(x = samples, list = !duplicated(x = silly), values = "Pop")) %>% # replace the now duplicated 'sample' with the word Pop.
    dplyr::select(-c("silly", "fish")) %>% # drop the extra columns
    replace(x = ., list = TRUE, values = Reduce(f = function(x, y) ifelse(x == "Pop", NA, y), x = ., acc = TRUE)) %>% #replace anything occurring after "Pop", rowwise, with NA
    as.matrix() #back to matrix
  
  #write output file
  write("Title Line:",filename)
  write(colnames(genepopBases[,-1]),filename,ncolumns=1,append=TRUE) #dropping first column, 'samples'
  #write("Pop",filename,append=TRUE)
  write.table(genepopBases,filename,quote=FALSE,sep="\t",col.names=FALSE,append=TRUE,na = "", row.names = FALSE) #note-removing NAs and no rownames, because export icludes a name column
}
...
if(!require("pacman")) install.packages("pacman");library(pacman) # install and load pacman
p_load(msa, Biostrings, tools, utils) # load or install+load required packages.
...
genotypeSex<-function(locusReads,sexLocus,controlLocus,p_thresh=0.001,epsilon=0.01,plotGenotypes="N",saveDir=NULL){
